...
def printEpisodesDetailed(query,desc=2):
    '''
...
def printMovies(query,order=1):
    '''
    Function that prints out a movie or list of movies, according to a user-made query
    :param query: string, a sql query that returns entries in the movies table
    :param order: integer, 1 is by title, 2 by episode, 3 by year, 4 by director
    '''
    con = sqlite3.connect('botw.db')
    cursor = con.cursor()
    cursor.execute(query)
    ...
    print(len(rows), ' results found.')
    # Query order is title, year, director, episode
    if order == 1: # By title
        for r in rows:
            line = r[0]
            if r[1] != None:
            ...
            if r[2] != None:
                line = line + ' Dir. ' + r[2]
            print(line + ' (ep. ' + str(r[3]) +')')
    elif order == 2: # By episode
        for r in rows:
            line = 'Ep. ' + str(r[3]) + ' - ' + r[0]
            if r[1] != None:
                line = line +  ' (' + str(r[1]) + ')'
            if r[2] != None:
                line = line + ' Dir. ' + r[2]
            print(line)
    elif order == 3: # By year
        for r in rows:
            line = str(r[1]) + ' ' + r[0]
            if r[2] != None:
                line = line + ' (Dir. ' + r[2] + ')'
            print(line + ' (ep. ' + str(r[3]) + ')')
    elif order == 4: # By director
        for r in rows:
            line = r[2] + ' - ' + r[0]
            if r[1] != None:
                line = line + ' (' + str(r[1]) + ')'
...
def lookForHosts():
    ...
    query = 'SELECT e.number, e.title, e.upload, e.length from people p join hosts_episodes h JOIN episodes e on h.p_id = p.p_id and h.ep_number = e.number where p.name like \'%' + str(hosts[0]) + '%\''
    if len(hosts) > 1:
        for i in range(1,len(hosts)):
            query = query + 'INTERSECT SELECT e.number, e.title, e.upload, e.length from people p join hosts_episodes h JOIN episodes e on h.p_id = p.p_id and h.ep_number = e.number where p.name like \'%' + str(hosts[i]) + '%\''
    print('\n')
    printEpisodesDetailed(query=query)

# def dbEdit():
#     '''
#     Handles option 7 in the main menu, editing the database
#     '''
